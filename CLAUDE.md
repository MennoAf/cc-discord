...
- **Stop hook fires before Claude flushes the final assistant entry.** `_on_stop` retries the transcript read (`_read_final_assistant_text_with_retry`) for up to ~2s — without that retry, partial-write JSON lines fail to parse and the response never reaches Discord. Tests override `_STOP_TRANSCRIPT_RETRY_SECS` to skip the wait when verifying empty-text branches.
